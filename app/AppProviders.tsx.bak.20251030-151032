  ...
  useEffect(() => {
    const handle = (url?: string | null) => {
      ...
      if (route.includes("grant") && queryParams && typeof queryParams.id !== "undefined") {
        let ids = String(queryParams.id).split(",").map(s => s.trim()).filter(Boolean);
        if (ids.length === 1 && ids[0] === "all") {
          ids = [
            "theme:neon","theme:starry","theme:pink","theme:dark","theme:mint","theme:glitter",
            "theme:blackgold","theme:crimson","theme:emerald","theme:neonpurple","theme:silver",
            "cursor:glow","cursor:orb","cursor:star-trail"
          ];
        }
        ids.forEach(id => { if (id.startsWith("cursor:")) setCursorById(id as any); });
        grant(ids).catch(()=>{});
      }
    };
    Linking.getInitialURL().then(handle).catch(()=>{});
