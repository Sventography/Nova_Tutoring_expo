...
export const ISLAND_MILESTONES: IslandMilestone[] = [
   ...
   * CENTRAL ISLAND
   *
   * The original island fills completely before any new land grows outward.
   * Locked names are hidden by the Island screen, but the IDs remain stable
   * so future quests, companions, and interactions can target each location.
   */
  {
    id: "study_grove",
    level: 1,
    ...
    shortTitle: "Grove",
    description:
      "The first living patch of every Nova Island.",
    icon: "leaf-outline",
  },
  {
    id: "starlight_garden",
    ...
    shortTitle: "Garden",
    description:
      "A glowing garden that brightens as you keep learning.",
    icon: "flower-outline",
  },
  {
    id: "nova_library",
    ...
    shortTitle: "Library",
    description:
      "A luminous home for lessons, flashcards, and collections.",
    icon: "library-outline",
  },
  {
    id: "whisperwind_mill",
    level: 4,
    title: "Whisperwind Mill",
    shortTitle: "Windmill",
    description:
      "Its enchanted sails gather learning energy from the sky.",
  ...
  },
  {
    ...
    description:
      "A luminous waterfall powered by steady learning XP.",
    icon: "water-outline",
  },
  {
    id: "moonwell",
    ...
    shortTitle: "Well",
    description:
      "A wishing well whose water reflects distant constellations.",
  ...
  },
  {
    ...
    description:
      "A tower for goals, statistics, and discoveries among the stars.",
    icon: "planet-outline",
  },
  {
    ...
    shortTitle: "Habitat",
    description:
      "A living home where bonded companions can eventually roam.",
    icon: "paw-outline",
  },
   ...
   * OUTER REALMS
   *
   * These landmarks and their landmasses do not exist visually until the
   ...
   */
  {
    ...
    shortTitle: "Castle",
    description:
      "A fortified dream-castle protected by a luminous moat and bridge.",
  ...
  },
  {
    ...
    shortTitle: "Starport",
    description:
      "A sky-facing dock where a Nova spacecraft rests between journeys.",
    icon: "rocket-outline",
  },
  {
    id: "crystal_wilds",
    ...
    shortTitle: "Crystals",
    description:
      "An untamed floating wilderness filled with singing crystals.",
  ...
  },
  {
    ...
    shortTitle: "Temple",
    description:
      "A quiet celestial sanctuary built around an ancient moon gate.",
    icon: "moon-outline",
  },
];
...
type IslandContextValue = {
  ...
  grantDailyLoginXpIfNeeded: () => Promise<boolean>;
};
...
function stageForLevel(level: number): string {
  if (level >= 21) return "Celestial Dominion";
  if (level >= 18) return "Crystal Frontier";
  if (level >= 15) return "Starfaring Haven";
  if (level >= 12) return "Enchanted Realm";
...
function isProgressAhead(
  candidate: Pick<ProgressSnapshot, "xp" | "level">,
  baseline: Pick<ProgressSnapshot, "xp" | "level">
): boolean {
  if (candidate.level !== baseline.level) {
    return candidate.level > baseline.level;
  }

  return candidate.xp > baseline.xp;
}
...
export function IslandProvider({ children }: { children: ReactNode }) {
  ...
  /*
   * React effects can briefly call the daily-login function more than once
   * while providers finish hydrating. Sharing one in-flight promise prevents
   * duplicate reads and duplicate rewards.
   */
  const dailyLoginGrantRef =
  ...
  const refreshIsland = useCallback(async () => {
    ...
    try {
      ...
      let shouldSyncLocalProgressToRemote = false;
      ...
      if (userId) {
        ...
        if (!error && data) {
          const remoteProgress = {
            xp: safeXp(data.island_xp),
            level: safeLevel(data.island_level),
          };
          ...
          /*
           * Keep whichever snapshot is farther ahead. This prevents a newer
           * local reward—such as the daily-login grant—from being erased when
           * Supabase is briefly stale or a prior remote write was delayed.
           */
          if (isProgressAhead(remoteProgress, localProgress)) {
            nextXp = remoteProgress.xp;
            nextLevel = remoteProgress.level;
          } else {
            nextXp = localProgress.xp;
            nextLevel = localProgress.level;

            shouldSyncLocalProgressToRemote =
              isProgressAhead(localProgress, remoteProgress);
          }
        } else if (error) {
      ...
      if (shouldSyncLocalProgressToRemote) {
        await persistRemote(
          snapshot.xp,
          snapshot.level
        );
  ...
  const grantDailyLoginXpIfNeeded =
    useCallback(async (): Promise<boolean> => {
      /*
       * Never award against the temporary pre-hydration snapshot. The caller
       * also waits for ready, but this guard protects every future call site.
       */
      if (!ready) {
        return false;
      }
      ...
      const task = (async (): Promise<boolean> => {
        try {
          ...
          const key =
            `${DAILY_XP_KEY}:${userId || "guest"}`;
          const lastDate =
            await AsyncStorage.getItem(key);

          const currentToday =
            normalizeToday(
              progressRef.current.today
            );

          /*
           * Older builds could write today's date and then lose the XP when
           * hydration finished. Treat the reward as complete only when both
           * the date marker and today's login breakdown confirm the grant.
           */
          if (
            lastDate === todayKey &&
            currentToday.login > 0
          ) {
            return false;
          }

          /*
           * Save the XP first. The date marker is written only after the XP
           * operation completes, so a failed grant cannot permanently consume
           * that day's reward.
           */
          await addIslandXp(
            5,
      ...
      dailyLoginGrantRef.current = task;
      ...
      try {
        ...
        if (
          ...
          dailyLoginGrantRef.current = null;
        }
      }
    }, [
      addIslandXp,
