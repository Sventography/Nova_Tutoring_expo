...
export default function QuizScreen() {
  ...
  useEffect(() => {
    Animated.timing(prog, { toValue: totalQ ? answered/totalQ : 0, duration: 220, easing: Easing.out(Easing.cubic), useNativeDriver: false }).start();
  }, [answered, totalQ]);
  ...
  const startForTopic = (t: TopicIndex) => {
    ...
    if (!raw || !raw.length) return Alert.alert("No cards", "This topic has no cards yet.");
    const built = buildQuiz(raw.map(c => ({ q: c.q, a: c.a, choices: c.choices })), QUIZ_LEN);
    setMc(built); setIdx(0); setCorrect(0); setSelectedIdx(null); setLocked(false); setFinished(false); setStarted(true); timer.reset();
  };
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      ...
      else { setIdx(i=>i+1); setSelectedIdx(null); setLocked(false); }
    }, FEEDBACK_DELAY);
  ...
  const onPick = (choiceIndex: number) => {
    ...
    const current = mc[idx]; if (!current) return;
    setLocked(true); setSelectedIdx(choiceIndex);
    if (choiceIndex === current.correctIndex) { setCorrect(c=>c+1); Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium); }
    else { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); }
  };

  const restart = () => { setStarted(false); setFinished(false); setCorrect(0); setIdx(0); setSelectedIdx(null); setLocked(false); timer.reset(QUIZ_SECONDS); };
    ...
    <View style={S.container}>
      ...
      <View style={S.header}>
        ...
        <View style={S.row}>
          ...
          <View style={S.scorePill}><Ionicons name="trophy-outline" size={16} color="#FFE7A8" /><Text style={S.scoreTxt}>{correct}/{totalQ}</Text></View>
        </View>
        <View style={S.progWrap}><Animated.View style={[S.progFill, { width: progWidth }]} /></View>

        <FlatList horizontal data={topics} keyExtractor={t=>t.id} showsHorizontalScrollIndicator={false} contentContainerStyle={{paddingVertical:8}}
          renderItem={({item}) => (
            <Pressable onPress={() => startForTopic(item)} style={[S.topicPill, sel?.id===item.id && started && !finished && S.topicPillOn]}>
              <Text style={[S.topicTxt, sel?.id===item.id && started && !finished && S.topicTxtOn]}>{item.title} • {item.count}</Text>
            </Pressable>
          )} />
      </View>

      <View style={S.stage}>
        {!started && !finished && (<View style={S.center}><Text style={S.hint}>Tap a topic above to start a 5-minute quiz.</Text></View>)}
          ...
          <View style={S.card}>
            ...
            <View style={S.choices}>
              {current.options.map((opt, i) => {
                const isSel = selectedIdx === i, isRight = i === current.correctIndex;
                const showGreen = isSel && isRight, showRed = isSel && !isRight;
                return (
