...
export default function HeaderBar() {
  ...
  return (
    <View style={styles.wrap}>
      <LinearGradient colors={["#000000", "#0c1830"]} style={styles.container}>
        <View style={styles.left}>
          <Pressable onPress={() => router.push("/(tabs)/account")} style={styles.avatarWrap} hitSlop={8}>
            <Image
              key={avatarVersion}
              source={avatarSource}
          ...
          <View style={styles.coinPill}>
            {(() => {
              try { return <Image source={require("../assets/coin.png")} style={styles.coinIcon} />; }
              catch { try { return <Image source={require("../../assets/coin.png")} style={styles.coinIcon} />; }
        ...
        <View style={styles.right}>
          {(() => { const { enabled, toggle } = useFx(); return (
            <Pressable onPress={toggle} hitSlop={8} style={({pressed})=>[styles.fxBtn,pressed?{opacity:0.85}:null]}>
              <Ionicons name={enabled? "sparkles" : "sparkles-outline"} size={18} color={enabled? "#00e5ff" : "#cfeff6"} />
            </Pressable>
          ); })()}

          <Pressable onPress={onShare} hitSlop={8} style={({ pressed }) => [styles.shareBtn, pressed && { opacity: 0.7 }]}>
            <Ionicons name="share-outline" size={18} color="#cfeff6" />
          </Pressable>

          <Animated.View style={[styles.donateWrap, { transform: [{ scale }], shadowOpacity: glow as any }]}>
            <Pressable onPress={onDonate} hitSlop={10} style={({ pressed }) => [styles.donateBtn, pressed && { opacity: 0.85 }]}>
              <LinearGradient colors={["#0b2a5a", "#000000"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.donateGrad}>
...
const createStyles = (tokens: any) => StyleSheet.create({
  fxBtn:{height:32,minWidth:36,paddingHorizontal:10,borderRadius:16,backgroundColor: tokens.card as any,borderWidth:1,borderColor:"rgba(0,229,255,0.5)",alignItems:"center",justifyContent:"center",marginRight:8},
  ...
  avatar: { width: 32, height: 32, borderRadius: 16, marginRight: 8, backgroundColor: tokens.card as any },
  username: { color: tokens.text as any, fontWeight: "600", marginRight: 6 },
  coinPill: { flexDirection: "row", alignItems: "center", backgroundColor: tokens.card as any, borderRadius: 16, paddingHorizontal: 10, paddingVertical: 5 },
  coinIcon: { width: 16, height: 16, marginRight: 6, resizeMode: "contain" },
  coinText: { color: tokens.text as any, fontWeight: "700" },
  right: { flexDirection:"row", alignItems:"center", justifyContent:"flex-end" },
  shareBtn: { backgroundColor: tokens.card as any, borderRadius: 16, padding: 6, marginLeft: 8 },
  ...
  donateText: { color: tokens.text as any, fontWeight: "700", marginLeft: 6 },
