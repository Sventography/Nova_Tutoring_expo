...
function useTimer(seconds: number, running: boolean, onDone: () => void) {
  ...
  return {
    remaining,
    label: `${mm}:${ss}`,
    reset: (n = seconds) => setRemaining(n),
  };
}
    ...
    <View style={S.container}>
      ...
      <View style={S.header}>
        ...
        <View style={S.row}>
          <View style={S.timerPill}>
            ...
            <Text style={S.timerTxt}>{timer.label}</Text>
          </View>
      ...
      <View style={S.stage}>
        {!started && !finished && (
          <View style={S.center}>
            <Text style={S.hint}>
              Tap a topic above to start a 5-minute quiz.
            </Text>
          </View>
        )}
              ...
              {current.options.map((opt, i) => {
                ...
                return (
                  <Pressable
                    ...
                    style={[
                      S.choiceBtn,
                      showGreen && S.choiceBtnCorrect,
                      showRed && S.choiceBtnWrong,
                      locked && !isSel && S.choiceBtnDim,
                    ]}
                  >
                    <Text
                      style={[
                        S.choiceTxt,
                        (showGreen || showRed) && S.choiceTxtOn,
                      ]}
                    >
                      {opt}
                    </Text>
        ...
        {(finished || (started && !current)) && (
          <View style={S.center}>
            <Text style={S.resultTitle}>
              {timer.remaining === 0 ? "Time’s up!" : "Quiz finished!"}
            </Text>
...
export const S = StyleSheet.create({
  ...
  topicTxt: { color: "#9CC6DC", fontWeight: "600" },
  topicTxtOn: { color: "#E6FBFF", fontWeight: "800" },
  stage: { flex: 1, paddingHorizontal: 16, paddingTop: 8 },
  center: { flex: 1, alignItems: "center", justifyContent: "center", gap: 14 },
  hint: { color: "#9DC8E0" },
  card: { gap: 14, marginTop: 8 },
  qLabel: { color: "#A9D6EA", fontWeight: "700" },
  qText: { color: "#EAFBFF", fontSize: 18, fontWeight: "700" },
