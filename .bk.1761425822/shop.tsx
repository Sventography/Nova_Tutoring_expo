...
import { View, Text, ScrollView, Pressable, Linking as RNLinking, Platform, Image, LayoutAnimation, Animated } from "react-native";
...
/* ---------- RUNTIME-SAFE imports for components (default or named) --------- */
...
function canonId(id: string | null | undefined): string {
  ...
  // add missing prefixes if someone stored bare keys like "star_trail" or "neon"
  if (!v.includes(":")) {
  ...
  if (v === "theme:black-gold") v = "theme:black_gold";
  if (v === "theme:neon-purple") v = "theme:neon_purple";
  return v;
}
...
const track = (event: string, props?: Record<string, any>) => {
  try { (globalThis as any).novaTrack?.(event, props ?? {}); } catch {}
};
...
async function loadCursor(): Promise<string | null> { return (await AsyncStorage.getItem(CURSOR_KEY)) || null; }
async function saveCursor(key: string | null) { key ? await AsyncStorage.setItem(CURSOR_KEY, key) : await AsyncStorage.removeItem(CURSOR_KEY); }
async function loadTheme(): Promise<string | null> { return (await AsyncStorage.getItem(THEME_KEY)) || null; }
async function saveTheme(id: string | null) { id ? await AsyncStorage.setItem(THEME_KEY, id) : await AsyncStorage.removeItem(THEME_KEY); }
...
async function startCheckoutRequest(opts: { priceId?: string; amount?: number; currency?: string; success_url?: string; cancel_url?: string }) {
  ...
  track("shop_checkout_start", { base, ...opts });
  ...
  if (!res.ok) {
    track("shop_checkout_error", { error: json?.error ?? "unknown", status: res.status });
    throw new Error(json.error || "checkout failed");
  }
  const url = json.url as string;
  track("shop_checkout_url", { url });
  if (Platform.OS === "web") (window as any).location.assign(url);
...
function Section({
  ...
  children,
  pulseAnim,
}: { title: string; children: React.ReactNode; pulseAnim?: Animated.Value | null }) {
  return (
...
export default function Shop() {
  ...
  const cursorApi = (() => { try { return useCursor(); } catch { return null as any; } })();
  const setCursorById = cursorApi?.setCursorById as undefined | ((id: string | null) => void);
  const setThemeById = ((() => { try { return useTheme().setThemeById } catch { return undefined } })()) as undefined | ((id: string | null) => void);
  ...
  useEffect(() => {
    const onUrl = (event: { url: string }) => {
      ...
      if (it.category === "theme" || it.category === "cursor" || it.category === "bundle") {
        setPurchases((prev) => { const cid = canonId(it.id); const next = { ...prev, [it.id]: true } as PurchaseMap; if (cid && cid !== it.id) (next as any)[cid] = true;
          savePurchases(next);
  ...
  useEffect(() => {
    const t = setInterval(async () => {
      try { const c = await loadCoins(); setCoins(c); } catch {}
    }, 800);
  ...
  function toThemeCtxId(id: string | null) {
    ...
    const map: Record<string, string> = {
      "theme:neon": "neon",
      "theme:starry_night": "starry_night",
      "theme:pink": "pink",
      "theme:dark": "dark",
      "theme:mint": "mint",
      "theme:glitter": "glitter",
      "theme:black_gold": "black_gold",
      "theme:crimson": "crimson",
      "theme:emerald": "emerald",
      "theme:neon_purple": "neon_purple",
      "theme:silver": "silver",
    };
...
function markOwned(id: string) {
  ...
  const THEMES_MENU: QuickItem[] = useMemo(() => {
    const p = purchases; const eq = equippedTheme;
    return [
      { id: "theme:neon",         name: "Neon Nova",    kind: "theme",  owned: !!p["theme:neon"],         equipped: eq === "theme:neon" },
      { id: "theme:starry_night", name: "Starry Night", kind: "theme",  owned: !!p["theme:starry_night"], equipped: eq === "theme:starry_night" },
      ...
      { id: "theme:black_gold",   name: "Black & Gold", kind: "theme",  owned: !!p["theme:black_gold"],   equipped: eq === "theme:black_gold" },
      ...
      { id: "theme:neon_purple",  name: "Neon Purple",  kind: "theme",  owned: !!p["theme:neon_purple"],  equipped: eq === "theme:neon_purple" },
  ...
  const CURSORS_MENU: QuickItem[] = useMemo(() => {
    const p = purchases; const eq = equippedCursor;
    return [
  ...
  function quickEquip(id: string, kind: "theme" | "cursor") {
    ...
    if (kind === "theme") { isCurrentlyEq ? unequipThemeImmediate({ source: "quick_row" }) : equipThemeImmediate(id, { source: "quick_row" }); }
    else { isCurrentlyEq ? void unequipCursorImmediate({ source: "quick_row" }) : void equipCursorImmediate(id, { source: "quick_row" }); }
  }

  // “Unlock” now navigates to the correct section
  function quickBuy(id: string) {
    const kindGuess: "theme" | "cursor" | "other" = id.startsWith("theme:") ? "theme" : id.startsWith("cursor:") ? "cursor" : "other";
    track("shop_quick_action", { action: "unlock_click", id, kind: kindGuess, source: "quick_row" });
    if (kindGuess === "theme") { scrollTo(themeSectionY.current, { section: "themes" }); return; }
    if (kindGuess === "cursor") { scrollTo(cursorSectionY.current, { section: "cursors" }); return; }
    ...
    setNeed(1000); setShowInsufficient(true);
  ...
  const groups = useMemo(() => {
    const byCat: Record<Category, typeof catalog> = { plushies: [], clothing: [], tangibles: [], cursor: [], theme: [], bundle: [], coin_pack: [] };
  ...
  function buyWithCoins(it: any, meta?: { size?: string }) {
    ...
    const cid = canonId(it.id); const nextPurch = { ...purchases, [it.id]: true, [cid]: true } as PurchaseMap;
  ...
  const renderItem = (it: any, color: string, equipable?: "theme" | "cursor") => {
    ...
    const equipped =
      equipable === "theme" ? equippedTheme === it.id
      : equipable === "cursor" ? equippedCursor === it.id
      : false;
    ...
    return (
      <Card key={it.id} color={color}>
        {src ? (
          <Pressable
            ...
            style={{
              ...
              borderWidth: 1, borderColor: color, marginBottom: 8,
            }}
          >
        ...
        <Text style={{ color: "#eaf7fb", fontSize: 14, fontWeight: "700", textAlign: "center" }}>{it.title}</Text>

        {it.desc ? (
          <Text style={{ color: "#b0c9cf", fontSize: 12, lineHeight: 16, textAlign: "center", marginTop: 16, paddingHorizontal: 8 }} numberOfLines={3}>
            {it.desc}
          </Text>
        ) : null}
        ...
        {sizes.length > 0 ? (
          <View style={{ marginTop: 10 }}>
            <Text style={{ color: "#bfefff", fontSize: 12, marginBottom: 6 }}>Size</Text>
            <SizeSelector
              sizes={sizes}
              value={selected}
              onChange={(s: any) => { sizeCtl.set(sizeKey, s); track("shop_size_change", { sku: it.id, sizeKey, size: s }); }}
            />
          </View>
        ) : null}
        ...
        {equipable === "theme" ? (
          owned ? (
            <Pressable
              ...
              style={({ pressed }) => ({
                alignItems: "center", paddingVertical: 10, borderRadius: 10, borderWidth: 1,
                borderColor: "#5cfcc8", backgroundColor: pressed ? "rgba(92,252,200,0.15)" : "rgba(92,252,200,0.08)",
              })}
            >
              <Text style={{ color: "#5cfcc8", fontWeight: "800" }}>{equipped ? "Equipped ✓" : "Equip"}</Text>
            </Pressable>
          ) : (
            <View style={{ flexDirection: "row", justifyContent: "space-between", columnGap: 8 }}>
              <Pressable
                ...
                style={({ pressed }) => ({
                  ...
                  borderColor: color, backgroundColor: "transparent", opacity: pressed ? 0.9 : 1,
                })}
              >
          ...
          <Pressable
            ...
            style={({ pressed }) => ({
              alignItems: "center", paddingVertical: 10, borderRadius: 10, borderWidth: 1,
              borderColor: color, backgroundColor: pressed ? "rgba(245,158,11,0.15)" : "rgba(245,158,11,0.08)",
            })}
          >
            <Text style={{ color: color, fontWeight: "800" }}>${it.priceUSD?.toFixed(0)}</Text>
          </Pressable>
    ...
    <LinearGradient colors={["#000000", "#001528"]} style={{ flex: 1 }}>
      ...
      >
        ...
        >
          ...
          >
            <Text style={{ color: "#cfeaf0", fontSize: 14, fontWeight: "700" }}>{(coins ?? 0).toLocaleString()} coins</Text>
          </View>
        </View>
      ...
      <InsufficientCoinsModal
        ...
        needed={need}
        onClose={() => { setShowInsufficient(false); track("shop_modal_insufficient_closed"); }}
        onBuyCoins={() => {
...
function handleEquipTheme(id:string,setThemeById:any){
  setThemeById(id);
  AsyncStorage.setItem("@nova/themeId",id);
}
