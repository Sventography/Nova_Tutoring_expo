#!/usr/bin/env python3
# Nova Tutoring passive legendary installer v2.
#
...
'''
    ...
    # Toast wording differs across quiz revisions. It is presentation-only,
    # so do not abort the functional installation when the exact old block is
    # absent. Patch known forms when possible and otherwise preserve it.
    if "Aetherwyrm added +${reward.aetherwyrmBonus}" not in text:
        if old_toast in text:
            text = text.replace(old_toast, new_toast, 1)
        else:
            toast_pattern = re.compile(
                r"showToast\(\{(?:(?!showToast\().)*?message:\s*['\"]Correct answer!['\"](?:(?!showToast\().)*?\}\);",
                flags=re.DOTALL,
            )
            toast_match = toast_pattern.search(text)

            if toast_match:
                text = (
                    text[: toast_match.start()]
                    + new_toast
                    + text[toast_match.end() :]
                )
            else:
                print(
                    "⚠️  Optional quiz toast text was left unchanged."
                )
    ...
    old_notice = '''          setLegendaryNotice(
            `Astral Nova awakened · +${astralCertificateBonus} bonus coins`
          );'''
    new_notice = '''          setLegendaryNotice(
    ...
    old_certificate_message = (
        "              message: `Certificate bonus: "
        "+${astralCertificateBonus} coins`,"
    )
    new_certificate_message = (
        "              message: `Certificate bonus: "
        "+${certificateReward.totalCoins} coins`,"
    )

    if old_certificate_message in text:
        text = text.replace(
            old_certificate_message,
            new_certificate_message,
            1,
...
def main() -> int:
    try:
        print("🔧 Running passive legendary installer v2...")
