...
INITIAL_INVENTORY = {
    "plushie_nova_devil": {"limit": 500, "purchased": 0, "title": "Nova Plush Head (Devil Bow)"},
}

def load_db():
    if not os.path.exists(DB_PATH):
        save_db(INITIAL_INVENTORY)
...
def list_inventory():
    db = load_db()
    out = {k: {**v, "remaining": max(0, v["limit"] - v["purchased"])} for k, v in db.items()}
    return jsonify(out)
...
def get_item(item_id):
    ...
    if item_id not in db:
        return jsonify({"error": "unknown_item"}), 404
    v = db[item_id]
...
def purchase():
    ...
    if qty <= 0:
        return jsonify({"error": "bad_qty"}), 400
    with LOCK:
        db = load_db()
        if item_id not in db:
            return jsonify({"error": "unknown_item"}), 404
        ...
        if remaining < qty:
            return jsonify({"ok": False, "reason": "sold_out", "remaining": remaining}), 409
        item["purchased"] += qty
        save_db(db)
        remaining = max(0, item["limit"] - item["purchased"])
...
def refund():
    ...
    with LOCK:
        ...
        if item_id not in db:
            return jsonify({"error": "unknown_item"}), 404
        item = db[item_id]
        item["purchased"] = max(0, item["purchased"] - qty)
        save_db(db)
        remaining = max(0, item["limit"] - item["purchased"])
        return jsonify({"ok": True, "remaining": remaining})
...
def dollars_to_cents(amount_usd: float) -> int:
