...
export function AchievementsProvider({
  ...
  const { activeCompanionId, activeCompanion } = useCompanion();
  ...
  // Active companion and ability.
  //
  // Normalize every available companion identifier down to letters/numbers.
  // This safely matches colon, underscore, hyphen, and legacy formats.
  const normalizeCompanionToken = useCallback(
  ...
  const activeBonusPercent = Number(
    activeCompanion?.ability?.bonusPercent ?? 0
  );

  const hasMechaOwl =
    activeAbilityType === "achievement_reward_bonus" ||
    activeCompanionToken.includes("mechaowl");

  const hasCelestra =
    activeAbilityType === "streak_milestone_bonus" ||
    activeCompanionToken.includes("celestra");

  const hasAetherwyrm =
    activeAbilityType === "global_coin_multiplier" ||
    activeCompanionToken.includes("aetherwyrm");
  ...
  const computeAchievementCoins = useCallback(
    (base: number, id: string): number => {
      ...
      if (hasMechaOwl) {
        ...
        amount = Math.round(amount * (1 + bonus));
      }

      if (meta?.group === "streaks" && hasCelestra) {
        const bonus =
          activeAbilityType === "streak_milestone_bonus" &&
          activeBonusPercent > 0
            ? activeBonusPercent
      ...
      if (__DEV__) {
        console.log("[Achievements] reward calculation", {
          id,
          base,
          finalAmount,
          activeCompanionId,
  ...
  const value = useMemo<AchievementsContextValue>(
    () => ({
      ...
      devResetAchievement,
      devAchievementRewardPreview: computeAchievementCoins,
      devAchievementCompanionDebug: {
        activeCompanionId,
        activeCompanionToken,
        activeAbilityType,
